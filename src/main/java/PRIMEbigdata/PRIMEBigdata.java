	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<Cluster> streamOfPairs = entities.rebalance().flatMap(new FlatMapFunction<EntityProfile, Cluster>() {
			...
			public void flatMap(EntityProfile se, Collector<Cluster> output) throws Exception {
				...
				for (Integer tk : cleanTokens) {
					Cluster cluster = new Cluster(tk, new HashSet<Node>());
					cluster.addInCollection(new Node(tk, se.getKey(), cleanTokens, new HashSet<>(), se.isSource(), true));
		...
		SingleOutputStreamOperator<Cluster> entityClusters = entityBlocks.reduce(new ReduceFunction<Cluster>() {
			...
			public Cluster reduce(Cluster n1, Cluster n2) throws Exception {
				...
				return new Cluster(n1.getTokenkey(), n1.getCollection());
			}
...
//								numberOfComparisons.add(1);
		...
		SingleOutputStreamOperator<String> prunedGraph = pairEntityBlock.rebalance().map(new MapFunction<Node, String>() {
